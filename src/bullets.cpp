...
void Bullet::update()
{
  ...
  BulletShape_.setPosition({CurrentPos_.x,
                            CurrentPos_.y - settings::BulletSpeed});
}

void Bullet::draw()
{
  Window_->draw(BulletShape_);
}

sf::Vector2f Bullet::getPosition() const
{
  return BulletShape_.getPosition();
}

void Bullets::notify(const sf::Event &e)
{
  assert(e.type == sf::Event::KeyPressed);
  if (e.key.code == sf::Keyboard::Space && BulletsInSpace_.size() < settings::MaximumBullet)
  {
